...
type WaveOptions = { color: string; color2: string; size: number; glow: number; speed: number }
...
function buildCage(radius: number, detail: number) {
    ...
    for (let i = 0; i < verts; i += 3) {
        ...
        for (let k = 0; k < 3; k++) {
            face[(i + k) * 3] = cx; face[(i + k) * 3 + 1] = cy; face[(i + k) * 3 + 2] = cz
            faceSeed[i + k] = s
        }
    }
...
class GlobeScene {
    ...
    private S: ReturnType<typeof settingsFor>

    constructor(container: HTMLElement, cfg: Config) {
        this.container = container
        this.cfg = cfg
        this.S = settingsFor(cfg)
        const S = this.S
        ...
        const cageColors = {
            ...
            uShimmerColor: { value: new THREE.Color(cfg.shimmer?.color || DEFAULTS.shimmer.color) },
    ...
    updateConfig(cfg: Config) {
        ...
        p.uSpread.value = S.spread; p.uIntensity.value = S.intensity; p.uWave.value = S.wave; p.uHoverArc.value = S.hoverArc
        p.uDotSize.value = S.dotSize; p.uWobble.value = S.wobble; p.uFlicker.value = S.flicker
        ...
        c.uShimmer.value = S.shimmer; c.uEdgeMix.value = S.edgeMix; c.uSweepMix.value = S.sweepMix
        c.uSweepAxis.value = S.sweepAxis; c.uSweepWidth.value = S.sweepWidth; c.uNetGlow.value = S.netGlow
        c.uHoverGlow.value = S.hoverGlow; f.uFill.value = S.hoverFill
        ...
        if (cfg.density !== prev.density) {
            this.group.remove(this.points); this.pointGeo.dispose()
            this.pointGeo = buildPoints(S.points)
            this.points = new THREE.Points(this.pointGeo, this.pointMat)
            this.points.frustumCulled = false; this.group.add(this.points)
        }
        if (cfg.cage?.detail !== prev.cage?.detail || cfg.cage?.spread !== prev.cage?.spread) {
            this.group.remove(this.cage); this.group.remove(this.panels)
            this.cageGeo.dispose(); this.panelGeo.dispose()
            const built = buildCage(S.radius * S.cage, S.detail)
...
function GlobeBase(props: Props) {
    const {
        dot = DEFAULTS.dot,
        net = DEFAULTS.net,
    ...
    const cfgRef = useRef<Config>({ dot, net, density, spin, spinDir, hoverOn, sizePercent, dots, cage, shimmer, waves, hover })
    ...
    useEffect(() => {
        ...
        const io = new IntersectionObserver(
            ([entry]) => { if (entry.isIntersecting) scene.resume(); else scene.pause() },
            { threshold: 0 }
        )
        io.observe(container)
        scene.start()

        return () => { ro.disconnect(); io.disconnect(); scene.dispose(); sceneRef.current = null }
    }, [])
    ...
    return (
        <div
            ref={containerRef}
            role="img"
            aria-label="Globo interactivo de puntos en malla de alambre"
...
export default function Globe(props: Partial<Props>) {
    return <GlobeBase {...(PRESET as Props)} {...(props as Partial<Props>)} />
}
